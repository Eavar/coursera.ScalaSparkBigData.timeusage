...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val headL = StructField(columnNames.head, StringType, nullable = false)
    val tailL = columnNames.tail.map(x => StructField(x, DoubleType, nullable = false))
    val res = headL :: tailL
    StructType(res)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    columnNames.foreach {column =>
      ...
      ) primary_needs = new Column(column) :: primary_needs
      else
        if (
          column.startsWith("t05") ||
          column.startsWith("t1805")
        ) work = new Column(column) :: work
        else
          if (
            column.startsWith("t02") ||
            ...
            column.startsWith("t18")
          ) other = new Column(column) :: other
    }
...
//    val ageProjection: Column = expr("IF(15 <= teage AND teage <= 22, 'young', IF(23 <= teage AND teage <= 55, 'active', 'elder'))").as("age")
    ...
    val ageProjection: Column = when($"teage" >= 15 && $"teage" <= 22, "young")
                     .otherwise(when($"teage" >= 23 && $"teage" <= 55, "active").otherwise("elder")).as("age")
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    summed.groupBy($"working", $"sex", $"age")
          .agg((round(avg($"primaryNeeds")*10)/10).as("primaryNeeds"), (round(avg($"work")*10)/10).as("work"), (round(avg($"other")*10)/10).as("other"))
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String =
    "SELECT working, sex, age, ROUND(AVG(primaryNeeds)*10)/10 AS primaryNeeds, ROUND(AVG(work)*10)/10 AS work, ROUND(AVG(other)*10)/10 AS other FROM " + viewName + " GROUP BY working, sex, age ORDER BY working, sex, age"
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] =
    timeUsageSummaryDf.map(x => TimeUsageRow(x.getAs[String]("working"),
                                             x.getAs[String]("sex"),
                                             x.getAs[String]("age"),
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(k => (k.working, k.sex, k.age))
          ...
          .map(x => TimeUsageRow(x._1._1, x._1._2, x._1._3, (x._2*10).round.asInstanceOf[Double]/10, (x._3*10).round.asInstanceOf[Double]/10, (x._4*10).round.asInstanceOf[Double]/10))
          .orderBy($"working", $"sex", $"age")
  }
}
