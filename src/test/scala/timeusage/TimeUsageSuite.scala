...
//    assert(summaryDf.count === 6872)
    val summaryDffirst = summaryDf.first()
    assert((summaryDffirst.getAs("primaryNeeds").asInstanceOf[Double]
          +summaryDffirst.getAs("work").asInstanceOf[Double]
          +summaryDffirst.getAs("other").asInstanceOf[Double]).round
    === 24)
//    println(summaryDf.count)
//    summaryDf.show()
  }
...
//    summaryDs.show()
    ...
    val summaryDf = TimeUsage.timeUsageSummary(primaryNeedsColumns, workColumns, otherColumns, initDf)
    val summaryDs = TimeUsage.timeUsageSummaryTyped(summaryDf)
    val finalDs = TimeUsage.timeUsageGroupedTyped(summaryDs)
